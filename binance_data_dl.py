...
from binance_ui import *


def download_binance_data_klines(refresh: bool = False):
    ...
    for pair in track(TOKEN_PAIRS, description="Filtering data into folders..."):
        ...
        if not os.path.exists(BINANCE_KLINES_COMBINED_DATA_DIR):
            print(f"[red]Directory {BINANCE_KLINES_COMBINED_DATA_DIR} not found, creating")
            os.makedirs(BINANCE_KLINES_COMBINED_DATA_DIR)
        ...
        combo_csv_path = os.path.join(BINANCE_KLINES_COMBINED_DATA_DIR, pair + "-1m-all.csv")
...
# def download_binance_data_tades(refresh: bool = False):
#     # download the monthly data (and daily data for the current month) for the listed pairs
#     # i.e. the trade information
#     data_dumper = BinanceDataDumper(BINANCE_DATA_DIR)
#     if not os.path.exists(BINANCE_DATA_DIR) or refresh:
#         data_dumper.dump_data(tickers=TOKEN_PAIRS)
...
#     for pair in track(TOKEN_PAIRS, description="Filtering data into folders..."):
#         # Make sure that the combined directory exists
#         if not os.path.exists(BINANCE_TRADES_COMBINED_DATA_DIR):
#             print(f"[red]Directory {BINANCE_TRADES_COMBINED_DATA_DIR} not found, creating")
#             os.makedirs(BINANCE_TRADES_COMBINED_DATA_DIR)

#         # path to CSV that will contain all the data for this pair combined across months
#         # no header row, but column names are:
#         # Open time,Open,High,Low,Close,Volume,Close time,Quote asset volume,Number of trades,Taker buy base asset volume,Taker buy quote asset volume,Ignore
#         # as specified here: https://github.com/binance/binance-public-data#trades
#         combo_csv_path = os.path.join(BINANCE_TRADES_COMBINED_DATA_DIR, pair + "-1m-all.csv")
#         with open(combo_csv_path, "w+") as combo_csv:
#             # Dump the headers
#             combo_csv.write(",".join(BINANCE_DATA_HEADER) + "\n")
